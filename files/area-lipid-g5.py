...
desc = """
 ...
 USAGE: %s <.edr file> <begin time (ps)> <end time (ps)> <#lipids in a monolayer> <Temprature (K)> > ener-area-out.xvg
...
procString = "area-lipid.py from file " + args[0] + " starting " + args[1] + " to " + args[2] + " lipid count " + args[3] + " temprature " + args[4]
print procString

# Parse input / define constants
ene_file = args[0]
begin_time = int(args[1])
end_time = int(args[2])
lipids_per_mon = float(args[3])
temprature = float(args[4])

ene_out_file = 'ener-area.xvg'
block_A0_out_file = 'ener-area-A0-block.xvg'
block_KA_out_file = 'ener-area-KA-block.xvg'
...
print >>outFile, '# ' + procString
print >>outFile, '# block averaging for A_0'
...
for block_size in block_range:
    ...
    print >>outFile, "%7i    %10.8f    %10.8f    %10.8f" % (block_size, numpy.average(blocked_A0), currentSTD, currentSE)
...
val_scaling = (scaled_k * temprature * val_A0) / lipids_per_mon  # kT A0 / N
...
print >>outFile, '# ' + procString
print >>outFile, '# block averaging for K_A'
block_max = int(line_count/5)
block_range = range(1, block_max, 5)
block_in_top_20 = block_max * 0.8
val_KA_sd = -1
